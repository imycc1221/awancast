...
The prototype is a React + Vite + TypeScript app in [`awan-cast/`](awan-cast/). It runs entirely in the browser with mocked forecasts by default — no backend or account needed.
...
The scheduler, tariff logic, and confidence framing shown in the UI are the same ones described above; the explanation agent is optional and off by default (`VITE_AGENT_ENABLED`).
...
├── awan-cast/          # Interactive prototype dashboard (React + Vite + TypeScript)
│   ├── src/            # Components, scheduler, tariff logic, i18n (EN/BM)
